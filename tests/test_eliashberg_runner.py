def test_units_conversion_cm1_to_mev(tmp_path):
    ...
    p = tmp_path / "a2F.csv"
    # Two peaks at 1000 and 1200 cm^-1 (≈124 & 149 meV)
    arr = np.array([[1000,1.0],[1200,0.8]], float)
    np.savetxt(p, arr, delimiter=",")
    cmd = [sys.executable, "quantum-rtsc-protocol/tools/eliashberg_runner.py", "--alpha2F_csv", str(p),
...
def test_multimodality_triggers_exit_code(tmp_path):
    ...
    r = subprocess.run([sys.executable, "quantum-rtsc-protocol/tools/eliashberg_runner.py",
